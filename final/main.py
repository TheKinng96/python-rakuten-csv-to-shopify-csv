...
try:
    ...
    if not REQUIRED_COLUMNS.issubset(raw_df.columns):
        raise KeyError(f"Input CSV is missing essential columns: {', '.join(REQUIRED_COLUMNS - set(raw_df.columns))}")
    raw_df['SKU'] = raw_df['商品管理番号（商品URL）'].str.strip()
    grouped = raw_df.groupby('SKU')
    merged_data = []
    for sku, group in grouped:
        if not sku:
            log_entry = group.copy(); log_entry['Reason'] = 'Blank SKU'; rejected_rows_log.append(log_entry); continue
        master_rows = group[group['商品名'].str.strip() != ''].copy()
        attribute_rows = group[group['商品名'].str.strip() == ''].copy()
        if master_rows.empty:
            log_entry = group.copy(); log_entry['Reason'] = 'Orphaned attribute rows'; rejected_rows_log.append(log_entry); continue
    ...
    if ss_sku_mask.any():
        rejected_df = item_df[ss_sku_mask].copy(); rejected_df['Reason'] = 'SKU ends with -ss'; rejected_rows_log.append(rejected_df); item_df = item_df[~ss_sku_mask]
...
if rejected_rows_log:
    ...
    with open(LOG_FILE, "w", newline="", encoding="utf-8") as f:
        log_header = original_columns + ['Reason']; all_rejected_df.to_csv(f, columns=log_header, index=False, header=True)
